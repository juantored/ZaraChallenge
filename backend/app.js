...
app.get('/mobiles', async (req, res) => {
  ...
  try {
    ...
    if (!response.ok) return res.status(response.status).json({ error: 'Error al cargar los móviles' });
...
app.get('/mobiles/:id', async (req, res) => {
  ...
  try {
    ...
    if (!response.ok) return res.status(response.status).json({ error: 'Error al cargar el móvil' });
